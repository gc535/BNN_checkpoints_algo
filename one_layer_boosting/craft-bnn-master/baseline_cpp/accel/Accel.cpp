...
void bin_conv(
  ...
  for (ap_uint<6> w = 0; w < words_per_image; ++w) {
    ...
    const ap_uint<6> out_offset = (w % POOL_AREA) << 4;
    ...
    if (norm_mode == 1) {
      ...
      ap_int<WORD_SIZE/POOL_WIDTH> poolword_h;
      for (ap_uint<6> i = 0; i < WORD_SIZE/POOL_WIDTH; ++i) {
        poolword_h[i] = binword[POOL_WIDTH*i] & binword[POOL_WIDTH*i+1];
      }

      // vertical pooling
      for (ap_uint<6> i = 0; i < WORD_SIZE/POOL_AREA; ++i) {
        // source indices
      ...
      if (log_width != LOG_BANK_WIDTH) {
        o_bank_offset /= POOL_AREA;
        outword = poolword;
      } else {
        outword = outword >> WORD_SIZE/POOL_AREA;
        outword(63,48) = poolword(15,0);
        o_bank_idx = (o_index/POOL_AREA)%CONVOLVERS;
        o_bank_offset = (o_index/POOL_AREA)/CONVOLVERS;
      }
    }
...
void top(
  ...
  if (layer_type == LAYER_CONV1) {
    ...
    assert(norm_mode != 2 || n_outputs % POOL_AREA == 0); // needed for pooling of 8x8 image
