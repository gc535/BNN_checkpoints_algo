...
int main(int argc, char** argv) {
  ...
  Word* data_o  = (Word*) MEM_ALLOC( DMEM_WORDS * sizeof(Word) );
  if (!data_i || !data_o) {
  ...
  for (unsigned n = 0; n < n_imgs; ++n) {
    ...
    for (unsigned l = lconv+1; l <= ldense; ++l) {
      ...
      if (DENSE_LAYER_CPU) {
        dense_layer_cpu(
            (uint64_t*)wt[l-1],
            params.float_data(3*l-2),
            params.float_data(3*l-1),
            (uint64_t*)(l%2==0 ? data_i : data_o),
            (uint64_t*)(l%2==0 ? data_o : data_o),
            M, N
        );
        ...
        run_accel_schedule(
            ...
            l-1,
            0,    // input_words
            (l==ldense && LAST_LAYER_CPU) ? 1024/WORD_SIZE : 0, // output_words
            l % 2,
            layer_sched[l-1]
        );
      }
